...
class Metrics(object):
    ...
    '''
    def __init__(self, lane_metrics, world_metrics, env):
        # must take record of rewards
        self.env = env
        self.decision_num = 0
        self.lane_metric_List = lane_metrics
        self.lane_metrics = dict()
        self.lane_metrics['rewards'] =  np.array([0 for _ in range(self.env.n_intersections)], dtype=np.float32)
        self.lane_metrics.update({k : np.array([0 for _ in range(self.env.n_intersections)], dtype=np.float32) for k in self.lane_metric_List})
    ...
    def clear(self):
        ...
        '''
        self.lane_metrics['rewards'] =  np.array([0 for _ in range(self.env.n_intersections)], dtype=np.float32)
        self.lane_metrics = {k : np.array([0 for _ in range(self.env.n_intersections)], dtype=np.float32) for k in self.lane_metric_List}
        self.decision_num = 0
    ...
    def delay(self):
        ...
        if 'delay' not in self.lane_metrics.keys():
            return self.env.world.get_real_delay()
        
        # apx_delay
        else:
            try:
                result = self.lane_metrics['delay']
                return np.sum(result) / (self.decision_num * self.env.n_intersections)
            except KeyError:
    ...
    def throughput(self):
        ...
        '''
        return self.env.world.get_cur_throughput()
    ...
    def real_average_travel_time(self):
        ...
        '''
        return self.env.world.get_average_travel_time()
